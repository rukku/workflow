...
def workflow(options):
    ...
    while config:
        ...
        for name, pattern, dt, action, ampersand in config:
            filenames = glob.iglob(f"{folder}/**/{pattern}", recursive=True)
            for filename in filenames:
                mt = os.path.getmtime(filename)
                if mt > time.time() - dt:
                    continue
                pid_file = os.path.basename(filename) + '.%s.pid' % name
                log_file = os.path.basename(filename) + '.%s.out' % name
                err_file = os.path.basename(filename) + '.%s.err' % name
                key = re.sub('\s+', ' ', pattern + '=' +
                             filename + ':' + action).strip()
